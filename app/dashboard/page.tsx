...
export default function Dashboard() {
  ...
  async function loadFinancials(hid: string, uid: string) {
    ...
    const { data: txs } = await supabase
      .from('transactions')
      .select('amount, split, categories(name, icon, color)')
      .eq('household_id', hid)
      .gte('date', firstDay)
      .lte('date', lastDay);
  ...
  async function loadUpcomingRecurrences(hid: string) {
    const { data } = await supabase
      ...
      .select('*, transactions(description, amount, categories(name, icon))')
      .eq('household_id', hid)
      .eq('active', true)
  ...
  async function loadGoals(hid: string, uid: string) {
    ...
    const inProgress = (data || []).filter(g => Number(g.current_amount) < Number(g.target_amount)).slice(0, 3);
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: '#666', marginTop: 4 }}>
                  {balance
                    ? (balance.iOwe
                      ? `Você deve R$ ${balance.amount.toFixed(2)} para Parceiro(a)`
                      : `Parceiro(a) deve R$ ${balance.amount.toFixed(2)} para você`)
                    : 'Vocês estão quites! 🎉'}
                </div>
              </div>
          ...
          {upcomingRecurrences.map((r) => {
            ...
            return (
              <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #f5f5f5' }}>
                <div>
                  <span style={{ fontSize: 14 }}>{tx?.categories?.icon || '🔁'} {tx?.description || '—'}</span>
